...
function initLoader() { 
   ...
   tl.set(".loading-screen .icon-box svg g.tm", {
      opacity: 0
   });
   ...
   tl.to(".loading-screen .icon-box svg g.icon path", {
      yPercent: 0,
      xPercent: 0,
      opacity: 1,
      duration: 0.8,
   ...
   tl.to(".loading-screen .icon-box svg g.tm", {
      opacity: 1,
      duration: 1.2,
      ease: Expo.easeInOut
   }, "<");
...
function pageTransitionIn() {
   ...
   tl.to(".loading-screen .loading-dark", {
      opacity: 1,
      duration: 1.2,
      ease: Expo.easeInOut
   },"<");
...
function pageTransitionInPortfolioSingle() {
   ...
   tl.to('[data-transition-portfolio-single-fade-alt]', {
      opacity: 0,
      duration: 0.2,
   }, "<");
}
...
// Animation - Page Enter
function pageTransitionOutPortfolioSingle() {
	var tl = gsap.timeline();
...
function initPageTransitions() {
   ...
   function initResetDataAfter() {
      ...
      $('[data-scrolling-transparant]').attr('data-scrolling-transparant', 'false');
...
function initBasicFunctions() {
   ...
   $('[data-hover-scramble]').each(function () {

      let tlScrambleHover = gsap.timeline({paused: true});
      
      tlScrambleHover.to($(this).find('[data-hover-scramble-target]'), {
         duration: 0.6,
         scrambleText: {
            text: "{original}", 
            speed: 1.2,
      ...
      $(this).on('mouseenter', function() {
         tlScrambleHover.play();
      });

      $(this).on('mouseleave', function() {
         tlScrambleHover.reverse();
      });

   });

   // Find all Writing Cards and add class on hover
   $(".writing-grid").each(function () {
      var gridWriting = $(this);
      gridWriting.find('[data-writing-card-status] a').on('mouseenter', function() {
         gridWriting.find('[data-writing-card-status]').attr('data-writing-card-status', 'not-active');
         $(this).parent().attr('data-writing-card-status', 'active');
      });
   });

   // Team Cards
   $('[data-team-card-id]').click(function(){
      var teamCardID = $(this).data('team-card-id');
      // If card == open, close it. Else: main action
      if ($('[data-team-lightbox-id="' + teamCardID + '"]').attr('data-team-lightbox-status') == 'active') {
         $('[data-team-lightbox-id="' + teamCardID + '"]').attr('data-team-lightbox-status', 'not-active');
      } else {
         $('[data-team-lightbox-id="' + teamCardID + '"]').attr('data-team-lightbox-status', 'active').siblings().attr('data-team-lightbox-status', 'transition');
         setTimeout(function() {
            $('[data-team-lightbox-id="' + teamCardID + '"]').siblings().attr('data-team-lightbox-status', 'not-active');
         }, 600);
         gsap.from($('[data-team-lightbox-id="' + teamCardID + '"]').find('.title'), {
            duration: 0.6,
            scrambleText:{
               text: "{original}", 
               speed: 1.2,
   ...
   $('[data-team-lightbox-toggle="close"]').click(function(){
      $('[data-team-lightbox-id]').attr('data-team-lightbox-status', 'not-active');
   });

   // Toggle Invest Lightbox
   $('[data-invest-lightbox-toggle="toggle"]').click(function(){
      if ($('[data-invest-lightbox-status]').attr('data-invest-lightbox-status') == 'not-active') {
         $('[data-invest-lightbox-status]').attr('data-invest-lightbox-status', 'active');
      } else {
         $('[data-invest-lightbox-status]').attr('data-invest-lightbox-status', 'not-active');
      }
   });
   
   // Close Invest Lightbox
   $('[data-invest-lightbox-toggle="close"]').click(function(){
      $('[data-invest-lightbox-status]').attr('data-invest-lightbox-status', 'not-active');
   });

   // Key ESC - Close Invest Lightbox
   $(document).keydown(function(e){
      if(e.keyCode == 27) {
         if ($('[data-invest-lightbox-status]').attr('data-invest-lightbox-status') == 'active') {
            $('[data-invest-lightbox-status]').attr('data-invest-lightbox-status', 'not-active');
         } 
      }
   });
   ...
   // Core Section Home
   $('[data-core-target]').on('mouseenter', function() {
      var coreID = $(this).data('core-id');
      if ($('[data-core-id="' + coreID + '"]').attr('data-core-status') == 'not-active') {
         $('[data-core-id]').attr('data-core-status', 'not-active');
         $('[data-core-id="' + coreID + '"]').attr('data-core-status', 'active');
      }
   });
...
function initSplitText() {
   ...
   var splitTextTypewriter = new SplitText(".split-typewriter", {type: "words, chars", wordsClass: "single-word", charsClass: "single-char"});
   
   var splitTextRead = new SplitText(".split-read", {type: "words", wordsClass: "single-word"});
}
   ...
   function startCheckScroll() {
      scroll.on('scroll', (instance) => {
         ...
         if (Math.abs(lastScrollTop - nowScrollTop) >= threshold) {
            ...
            // Check if NAV Transparant
            if (nowScrollTop > (threshold * 4)) {
               $("[data-scrolling-transparant]").attr('data-scrolling-transparant', 'true');
            } else {
               $("[data-scrolling-transparant]").attr('data-scrolling-transparant', 'false');
            }
         }
 ...
 */
 function initScrollTriggerDataBackground() {
   ...
   const navHeight = $(".main-navigation").innerHeight();
...
function initIsotopeFilter() {
   ...
   $('[data-single-filter-toggle="toggle"]').click(function(){
      if ($(this).parent().parent().attr('data-single-filter-status') == 'not-active') {
         $(this).parent().parent().attr('data-single-filter-status', 'active');
         $(this).parent().parent().parent().siblings().find('[data-single-filter-status]').attr('data-single-filter-status', 'not-active');
         gsap.to($(this).parent().parent().find('[data-hover-scramble-target]'), {
            duration: 0.6,
            scrambleText: {
               text: "{original}", 
               speed: 1.2,
         ...
         $(this).parent().parent().attr('data-single-filter-status', 'not-active');
      }
   });

   // Source: https://codepen.io/desandro/pen/MebyMR
   // Library Isotope: https://isotope.metafizzy.co/
   // Alternative: http://codepen.io/desandro/pen/c72ec4477f913f82d32f18e4a738a697

   // store filter per group
   var filters = {};
   var $container = $('[data-iso-container]');

   //  Disable Layout Mode, Source: https://github.com/metafizzy/isotope/issues/675#issuecomment-40675160
   ...
   // Disable layout changes,  Source: https://github.com/metafizzy/isotope/issues/675
   ...
   $('[data-iso-filters]').on( 'change', function( event ) {
      var checkbox = event.target;
      var $checkbox = $( checkbox );
      var group = $checkbox.parents('[data-filter-group]').attr('data-filter-group');
      ...
      tl.call(function() {
         $('[data-card-status]').attr('data-card-status', 'not-active');
         $(elems).each(function() {
            $(this).attr('data-card-status', 'active');
         });
...
function initScrolltriggerAnimations() {
   ...
   if(document.querySelector('.split-read')) {
      // Scrolltrigger Animation : Example
      $('.split-read').each(function (index) {
   ...
   if(document.querySelector("[data-scroll-typewriter]")) {
      // Scrolltrigger Animation : Example
      $('[data-scroll-typewriter]').each(function (index) {
         let triggerElement = $(this);
         let targetElement = $(this).find('.single-word, .single-char');
         ...
         tl.fromTo(targetElement, {
            ...
            ease: Expo.easeIn,
            onComplete: function() {
               triggerElement.find('.typewriter-bar').addClass('blink');
            }
         });
      });
   }

   $('[data-loop-scramble]').each(function () {
      let triggerElement = $(this);
      let targetElement = $(this);
